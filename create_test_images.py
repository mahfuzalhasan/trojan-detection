...
class CutPasteDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return image, aug_img_1, aug_img_2, cp, class_label 
...
if __name__ == "__main__":
    ...
    validation_image_path = "./data_split/07-11-22_1755/valid_images.pkl"
    valid_img_file = pickle.load(open(validation_image_path, 'rb'))

    valid_dataset = CutPasteDataset(valid_img_file, run_started)
    valid_dataloader = DataLoader(valid_dataset, batch_size = 1, shuffle=False)
    print("Valid dataset length: ",len(valid_dataset))
    print('Valid dataloader: ',len(valid_dataloader))

    run_id = "07-13-22_1357"

    save_path = os.path.join("./Test_Data", run_id)
    if not os.path.exists(save_path):
        os.makedirs(save_path)

    save_path_orig = os.path.join(save_path, "original")
    if not os.path.exists(save_path_orig):
        os.makedirs(save_path_orig)

    save_path_aug = os.path.join(save_path, "augmented")
    if not os.path.exists(save_path_aug):
        os.makedirs(save_path_aug)

    save_path_anom = os.path.join(save_path, "anomaly")
    if not os.path.exists(save_path_anom):
        os.makedirs(save_path_anom)

    for index, (image, aug_img_1, aug_img_2, anom, class_labels) in enumerate(valid_dataloader):
        orig_img_path = '%s/orig_%05d.jpg' % (save_path_orig, index)
        aug_img_1_path = '%s/aug_1_%05d.jpg' % (save_path_aug, index)
        aug_img_2_path = '%s/aug_2_%05d.jpg' % (save_path_aug, index)
        anom_path = '%s/anom_%05d.jpg' % (save_path_anom, index)

        torchvision.utils.save_image(image, orig_img_path, normalize=True, nrow=1, range=(0, 1))
        torchvision.utils.save_image(aug_img_1, aug_img_1_path, normalize=True, nrow=1, range=(0, 1))
        torchvision.utils.save_image(aug_img_2, aug_img_2_path, normalize=True, nrow=1, range=(0, 1))
        torchvision.utils.save_image(anom, anom_path, normalize=True, nrow=1, range=(0, 1))
